...
const Accordion = component$<AccordionRootProps>((props) => (
  // shadcn's AccordionRoot is animated by default, which is probably what most apps will want so I think it makes sense. Remember, this can be changed for the apps that don't want this behavior by default.
...
const AccordionTrigger = component$<AccordionTriggerProps>(({ ...props }) => {
  return (
    // shadcn wraps AccordionTigger with AccordionHeader here. I think it's better to keep the API similar to headless so that Accordions can change their Headers to a different html tag with the 'as' prop on a per Accordion basis.
